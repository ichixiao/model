...
def loglikelihood(dim,num0,r0,nu0,Chol0,num,r,nu,Chol):
    ...
    L = L-nu*sum(np.log(diag(Chol)))
    L = L+nu0*sum(np.log(diag(Chol0)))
global gammaterm_n
    if isnan(gammaterm_n[num])
        gammaterm_n[num] = 0
        for d in range(0,dim):
            gammaterm_n[num] = gammaterm_n[num]+math.log(math.gamma(0.5*(nu+1-d)))
            gammaterm_n[num] = gammaterm_n[num]-math.log(math.gamma(0.5*(nu0+1-d)))
...
def loglikelihood2(dim,num0,r0,nu0,Chol0,num,r,nu,Chol):
    ...
    L = L-0.5*dim*math.log(r)
    L = L+0.5*dim*math.log(r0)
    L = L-nu*sum(np.log(np.diag(Chol)))
    L = L+nu0*sum(np.log(np.diag(Chol0)))
    for d in range(0,dim):
